---
title: "`assess_gtfs` documentation"
...
public transit information. GTFS is a pre-requisite for undertaking routing
with public transit schedules.

The `assess_gtfs` Python package helps to reduce barriers to understanding the
...
### The History of `assess_gtfs`

`assess_gtfs` started out life as a module in the ONS Data Science Campus'
[`transport_performance`][tp] package. `assess_gtfs` is built upon and inspired
by the excellent [gtfs_kit][gtfskit] package. `assess_gtfs` was needed to
...
<p>Want to get up and running with `assess_gtfs` quickly?</p>
...
<p class="jumbotron-icon">🔎</p>
<p>Need more details on the methods/tools used within `assess_gtfs`?</p>
...
<p class="jumbotron-icon">📖</p>
<p>Requiring a technical reference covering the `assess_gtfs` API?</p>
...
<p>Want to contribute to the development of `assess_gtfs`?</p>
...
build on this package. You can find the `assess_gtfs` source code on
